...
// ════════════════════════════════════════════════════════════════════════════
// DESIGN TOKENS  (shared with pos_page.dart palette)
// ════════════════════════════════════════════════════════════════════════════
class _D {
...
class _DashboardPageState extends State<DashboardPage>
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AuthBloc, AuthState>(
      builder: (context, authState) {
        ...
        // If not authenticated, show login screen instead of "Authentication Required"
  ...
  Widget _buildOwnerDashboard(BuildContext context) {
    ...
    final role = authState is AuthAuthenticated 
        ? authState.user.roleName.toLowerCase()
        : '';
        ...
        child: Column(children: [
          ...
          Expanded(
            child: TabBarView(
              controller: _tabController,
              children: [
                _OverviewTab(onReload: _reload),
                _AnalyticsTab(onReload: _reload),
              ],
            ),
          ),
        ]),
      ...
      body: Center(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text('Please log in to access to dashboard',
...
class _CashierDashboard extends StatelessWidget {
  const _CashierDashboard({required this.user});
  final dynamic user;
          ...
          if (state is SalesLoading) {
            return const Center(
              child: CircularProgressIndicator(
                  color: _D.primary, strokeWidth: 2),
            );
          }
            ...
            children: [
              ...
              Row(children: [
                ...
                Expanded(child: _QuickAction(
                  ...
                  label: 'Products',
                  color: _D.info,
                  onTap: () => context.push(RouteNames.sales), // update to RouteNames.products when route exists
                )),
                const SizedBox(width: 10),
                Expanded(child: _QuickAction(
                  ...
                  label: 'Users',
                  color: _D.warn,
                  onTap: () => context.push(RouteNames.settings), // update to RouteNames.users when route exists
                )),
              ]),
              const SizedBox(height: 20),
              ...
              Row(
                ...
                children: [
                  ...
                  GestureDetector(
                    ...
                    child: Text('See all',
                        style: _D.ts(13,
                            weight: FontWeight.w600, color: _D.primary)),
                  ),
                ],
              ),
    ...
    return RefreshIndicator(
      ...
      child: BlocBuilder<SalesBloc, SalesState>(
        builder: (context, state) {
          ...
          final maxSale  = sales
              .map<double>((s) => (s.total as num).toDouble())
              .reduce((a, b) => a > b ? a : b);
          ...
          return ListView(
            ...
            children: [
              ...
              // Revenue bar mini-chart (last 7 sales)
              Text('Revenue — Last ${sales.length > 7 ? 7 : sales.length} Sales',
                  style: _D.ts(15, weight: FontWeight.w700)),
              const SizedBox(height: 12),
              _MiniBarChart(
                sales: sales.take(7).toList().reversed.toList(),
              ),
...
class _GreetingBanner extends StatelessWidget {
  ...
  final int salesCount, todayCount;
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(
            ...
            children: [
              Text(title,
                  style: _D.ts(16, weight: FontWeight.w700)),
              const SizedBox(height: 4),
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
              color: _D.dangerSoft, shape: BoxShape.circle),
